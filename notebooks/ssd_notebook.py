...
path = '../../../../../../../../datasets/data1/Athindran_datasets/ILSVRC/Data/VID/test/ILSVRC2016_test_00123000/'
image_names = sorted(os.listdir(path))

process_time = np.zeros((13,1))
network_time = np.zeros((13,1))
process_medium_time = np.zeros((13,1))
...
for lvar in range(13):
  ...
  if rbboxes.shape[0]>=1:
    start_time = time.time()
    y1 = int(rbboxes[0,0]*0.8*img.shape[0])
    x1 = int(rbboxes[0,1]*0.8*img.shape[1])
    y2 = min(int(rbboxes[0,2]*1.2*img.shape[0]),img.shape[0])
    x2 = min(int(rbboxes[0,3]*1.2*img.shape[1]),img.shape[1])
    #print(y2-y1,x2-x1)
    ...
    #visualization.plt_bboxes(medium_img, r2classes, r2scores, r2bboxes)
    ...
    r2classes, r2scores, r2bboxes, network_time[lvar] =  process_image(fovea,0.1,0.45,(75,75),False,False) 
    end_time = time.time()
    ...
    #visualization.plt_bboxes(fovea, r2classes, r2scores, r2bboxes)
